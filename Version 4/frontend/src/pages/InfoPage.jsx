    ...
    async function getEstimate(){
        ...
        if(bill && month && solarPanelType != -1){
            console.log(roofType);
                    ...
                    if(solarPanelType == 0){
                        ELECTRICITY_RATE = "solarCost=" +  "" + 0.268;
                        panelSize = panelSize + "1.825";
                    }else if(solarPanelType == 1){
                        ELECTRICITY_RATE = "solarCost=" +  "" + 0.312;
                        panelSize = panelSize + "1.93";
                    }else if(solarPanelType == 2){
                        ELECTRICITY_RATE = "solarCost=" +  "" + 0.27;
                        panelSize = panelSize + "2.0412";
                    }else if(solarPanelType == 3){
                        ELECTRICITY_RATE = "solarCost=" +  "" + 0.273;
                        panelSize = panelSize + "1.94";
                    }else if(solarPanelType == 4){
                        ELECTRICITY_RATE = "solarCost=" +  "" + 0.282;
                        panelSize = panelSize + "1.9987";
                    }
                    
                    const response = await fetch('http://127.0.0.1:5000/getEstimate?' + 
                            address + "&" + material + "&" + elecBill + "&" + elecMonth + "&" + ELECTRICITY_RATE + "&" + roofLevel + "&" + panelSize
                    );
            ...
            <main>
                <div className="left-and-right">
                    <div className = "left">
                        ...
                        <div>
                            {hasEstimate && <img className = "solarMapStyles" src = {"http://127.0.0.1:5000/giveImage?address=" + infoId + "&month=" + month}/>}
                        </div>
                    ...
                    <h3>Solar Panels</h3>
                        <div className = "cards">
                            <Card
                                compName="REC Group Alpha Pure 410 W  "
                                cost="2.68 per Watt"
                            ...
                            />
                            <Card
                                compName="Maxeon Solar Technologies"
                                cost="3.12 per Watt"
                                modelnum="Model: info; Maxeon 3 BLK-R 420W"
                                id = {1}
                                isSelected = {solarPanelType}
                                setSelected = {setSolarPanelType}
                            />
                            <Card
                                compName="Canadian Solar TOPHiKu6 450 W"
                                cost="2.7 per Watt"
                                modelnum="Model: info"
                                id = {2}
                                isSelected = {solarPanelType}
                            ...
                            />
                            <Card
                                compName="Panasonic EVERVOLT_430HK2"
                                cost="2.73 per Watt"
                                modelnum="Model: info"
                                id = {3}
                                isSelected = {solarPanelType}
                ...
                {hasEstimate && <h3 style={{textAlign: "center", paddingBottom: "10px"}}>RESULTS</h3>}
                {hasEstimate && <p style={{textAlign:"center", paddingBottom: "30px"}} >We estimate that your solar panels will be paid back in {dataObtained.paybackPeriod} years!</p>}
                
                <div className="results-display">
                        
                        {hasEstimate && <Fact info = "Total Yearly Cost:" num = {"$" + Math.floor(dataObtained.totalYearlyCost)}/>}
                        {hasEstimate && <Fact info = "Total Solar System Cost:" num = {"$" + Math.floor(dataObtained.totalInstallationCost)}/>}
                        {hasEstimate && <Fact info = "Ideal Number of Solar Panels:" num = {Math.ceil(dataObtained.numPanels)}/>}
                        {hasEstimate && <Fact info = "NPV in 25 Years:" num =  {Math.floor(dataObtained.npv)}/>}
                        {hasEstimate && <Fact info = "ROI for 15 years" num =  {Math.round(dataObtained.ROI, 2) + "%"}/>}
                        {hasEstimate && <Fact info = "Area (msq)" num =  {Math.round(dataObtained.area)}/>}
