...
def calculate_yearly_savings(total_area, yearly_kwh_per_4_panels, ELECTRICITY_RATE):
    ...
    return yearly_savings/6.0, num_panels
...
def test():
    ...
    wholeBuildingRoofArea = int(data['solarPotential']['buildingStats']['areaMeters2'])
    print(wholeBuildingRoofArea)
    print(wholeBuildingRoofArea/10.6)
    ...
    ROI = (yearly_savings * 15 - (total_installation_cost))/total_installation_cost
...
def get_image():
    ...
    month_value = int(request.args.get("month"))
    ...
    geoiff.display_image(f'https://solar.googleapis.com/v1/dataLayers:get?location.latitude={latitude}&location.longitude={longitude}&radiusMeters=100&view=FULL_LAYERS&requiredQuality=HIGH&exactQualityRequired=true&pixelSizeMeters=0.5&key={GOOGLE_KEY}', key=GOOGLE_KEY, band_index=month_value)
